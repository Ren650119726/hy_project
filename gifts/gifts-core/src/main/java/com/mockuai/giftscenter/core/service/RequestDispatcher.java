...
import com.mockuai.giftscenter.common.constant.ParamEnum;
import com.mockuai.giftscenter.common.constant.ResponseCode;
import com.mockuai.giftscenter.core.exception.GiftsException;
import com.mockuai.giftscenter.core.filter.FilterChain;
import com.mockuai.giftscenter.core.filter.FilterManager;
import com.mockuai.giftscenter.core.service.action.Action;
import com.mockuai.giftscenter.core.service.action.ActionHolder;
...
//	private LimitBarrier barrier = new LimitBarrier();
    ...
    public GiftsResponse dispatch(SeckillRequest req) {
        if (req == null) {
            throw new IllegalArgumentException("request is null!");
        }
        // 单例类限流措施
        ActionCall task = new ActionCall(req);
        GiftsResponse response = null;
        try {
...
//		}
     ...
     * </pre>
     */
    class ActionCall implements Callable<GiftsResponse> {
        ...
        public GiftsResponse call() {
            ...
            if (null != action) {
                ...
                GiftsResponse re = new GiftsResponse(true);
                //获取appCode
                String appCode = getAppCode(context, req);
                if (!allowAccess(appCode, action)) {
                    re = new GiftsResponse(ResponseCode.REQUEST_FORBIDDEN);
                    return re;
                }
                ...
                try {
                    ...
                    //3. 执行filter.after流程
                    GiftsResponse afterFilterResult = filterChain.after(context);
                    ...
                    res = new GiftsResponse(e.getCode(), e.getMessage());
                    log.error("do action:" + req.getCommand() + " occur Exception:", e);
                    return res;
                } catch (Exception e) {
                    res = new GiftsResponse(ResponseCode.SERVICE_EXCEPTION);
                    log.error("do action:" + req.getCommand() + " occur Exception:", e);
                    return res;
                } finally {
                ...
                GiftsResponse res = new GiftsResponse(ResponseCode.ACTION_NO_EXIST);
                return res;
            }
        }
