...
public final class GiftsUtils {
    ...
    public static <T> GiftsResponse<T> getSuccessResponse() {
        return new GiftsResponse(ResponseCode.SUCCESS);
    }


    public static <T> GiftsResponse<T> getSuccessResponse(T module, int total) {
        GiftsResponse response = new GiftsResponse(module, total);
        response.setResCode(ResponseCode.SUCCESS.getCode());
        response.setMessage(ResponseCode.SUCCESS.getMessage());
        return response;
    }

    public static <T> GiftsResponse<T> getSuccessResponse(T module) {
        return getSuccessResponse(module, 1);
    }

    public static <T> GiftsResponse<T> getFailResponse(int errorCode, String msg) {
        return new GiftsResponse(errorCode, msg);
    }

    public static <T> GiftsResponse<T> getFailResponse(ResponseCode responseCode) {
        return new GiftsResponse<T>(responseCode);
    }
