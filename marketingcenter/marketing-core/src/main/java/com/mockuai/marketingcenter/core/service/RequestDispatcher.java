...
import com.mockuai.marketingcenter.common.constant.ParamEnum;
import com.mockuai.marketingcenter.common.constant.ResponseCode;
import com.mockuai.marketingcenter.core.exception.MarketingException;
import com.mockuai.marketingcenter.core.filter.FilterChain;
import com.mockuai.marketingcenter.core.filter.FilterManager;
import com.mockuai.marketingcenter.core.service.action.Action;
import com.mockuai.marketingcenter.core.service.action.ActionHolder;
...
//	private LimitBarrier barrier = new LimitBarrier();
    ...
    public MarketingResponse dispatch(MarketingRequest req) {
        if (req == null) {
...
//		}
     ...
     * </pre>
     */
    class ActionCall implements Callable<MarketingResponse> {
        ...
        public MarketingResponse call() {
            ...
            if (null != action) {
                ...
                MarketingResponse re = new MarketingResponse(true);
                //获取appCode
                String appCode = getAppCode(context, req);
                if (!allowAccess(appCode, action)) {
                    re = new MarketingResponse(ResponseCode.REQUEST_FORBIDDEN);
                    return re;
                }
                ...
                try {
                    ...
                    //3. 执行filter.after流程
                    MarketingResponse afterFilterResult = filterChain.after(context);
                    ...
                    res = new MarketingResponse(ResponseCode.SERVICE_EXCEPTION);
                    log.error("do action:" + req.getCommand() + " occur Exception:", e);
                    return res;
                } finally {
                    accessLogger.info("command:{}, respCode:{}, filterBeforeCost:{}, actionCost:{}, filterAfterCost:{}" ,
                            req.getCommand(),
                            res.getResCode(),
                ...
                MarketingResponse res = new MarketingResponse(ResponseCode.ACTION_NO_EXIST);
                return res;
            }
        }
