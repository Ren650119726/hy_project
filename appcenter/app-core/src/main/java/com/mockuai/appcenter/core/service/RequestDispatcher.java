...
import com.mockuai.appcenter.common.constant.ParamEnum;
import com.mockuai.appcenter.common.constant.ResponseCode;
import com.mockuai.appcenter.core.exception.AppException;
import com.mockuai.appcenter.core.filter.FilterChain;
import com.mockuai.appcenter.core.filter.FilterManager;
import com.mockuai.appcenter.core.service.action.Action;
import com.mockuai.appcenter.core.service.action.ActionHolder;
...
//	private LimitBarrier barrier = new LimitBarrier();
	...
	public AppResponse dispatch(AppRequest req) {
		if(req == null){
...
//			logBizRt(req,startTime,response);
	...
	private String getAppCode(RequestContext context, AppRequest req) {
		//TODO impl
		String appCode = null;
...
//		}
	 ...
	 * </pre>
	 */
	class ActionCall implements Callable<AppResponse> {

		private AppRequest req;

		public ActionCall(AppRequest req) {
			super();
			this.req = req;
		}

		@Override
		public AppResponse call() {
			...
			if (null != action) {
				...
				AppResponse re = new AppResponse(true);
				//获取appCode
				String appCode = getAppCode(context, req);
				if (!allowAccess(appCode, action)) {
					re = new AppResponse(ResponseCode.BIZ_E_REQUEST_FORBIDDEN);
					return re;
				}
				...
				try {
					...
					res = new AppResponse(e.getResponseCode(), e.getMessage());
					log.error("do action:" + req.getCommand() + " occur Exception:", e);
					return res;
				} catch (Exception e) {
					res = new AppResponse(ResponseCode.SYS_E_SERVICE_EXCEPTION);
					log.error("do action:" + req.getCommand() + " occur Exception:", e);
					return res;
				}finally{
				...
				AppResponse res = new AppResponse(ResponseCode.PARAM_E_ACTION_NOT_EXIST);
				return res;
			}
		}
