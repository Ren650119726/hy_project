...
import com.mockuai.tradecenter.core.util.BizMarkCollectionUtil;
...
public class QueryUserCartItems implements Action {
	...
	public TradeResponse<List<DistributorCartDTO>> execute(RequestContext context) {
		...
		if (cartItems == null || cartItems.size() == 0) {
			...
			Map<Long, CartItemDO> idCartItemMap = new HashMap<>();
			...
			for (CartItemDTO cartItemDTO : cartItemDTOs) {
				if (cartItemDTO.getOriginalId() == null) {
					if (null != cartItemDTO.getItemType()
						...
						List<CartItemDTO> subCartItemList = new ArrayList<>();
					...
					if (itemSkuMap.containsKey(cartItemDTO.getItemSkuId()) == true) {
						...
						//设置购物车商品状态
						cartItemDTO.setStatus(getCartItemStatus(itemDTO));
						//设置购物车商品库存
//						cartItemDTO.setStockNum(getCartItemStock(itemSkuMap.get(cartItemDTO.getItemSkuId())));
						if(cartItemDTO.getNumber() > getSaleStockNum(cartItemDTO.getItemSkuId(),appKey).intValue()){
							cartItemDTO.setNumber(getSaleStockNum(cartItemDTO.getItemSkuId(),appKey).intValue());
						}
						
						cartItemDTO.setStockNum(getSaleStockNum(cartItemDTO.getItemSkuId(),appKey).intValue());
						
						//设置购物车商品名称和图片
						cartItemDTO.setItemName(itemDTO.getItemName());
//						cartItemDTO.setItemImageUrl(itemDTO.getIconUrl());
						// 查看sku图片
						if(StringUtils.isEmpty(itemSkuDTO.getImageUrl())){
							cartItemDTO.setItemImageUrl(itemDTO.getIconUrl());
						}else{
        ...
        try {
        	log.info(" >>>>>>> marketItemDTOs:"+JSONObject.toJSONString(marketItemDTOs));
        ...
        //设置标签 满减送 限时购
        BizMarkCollectionUtil.buildCartBizMarkCollection(discountInfoList,returnList);

        DistributorCartDTO distributorCartDTO = new DistributorCartDTO();
        distributorCartDTO.setDiscountInfoList(new ArrayList<DiscountInfo>());
        distributorCartDTO.setCartItems(returnList);
        module = Lists.newArrayList(distributorCartDTO);
...
//			List<ItemDTO> itemList = itemManager.queryItem(itemIdList, sellerId, appKey);
	...
	public String getName() {
		return ActionEnum.QUERY_USERCART_ITEMS.getActionName();
	}
