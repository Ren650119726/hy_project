 ...
 */
@Service
public class AlipayClientForRefund extends ClientExecutorAbstract  {

	private static final Logger log = LoggerFactory.getLogger(AlipayClientForRefund.class);
...
//		sParaTemp.put("notify_url", "http://74.207.229.184:8081/alipay/notify_url.jsp");
	...
	public PaymentUrlDTO getPaymentUrl(RequestContext context) throws TradeException {
		...
		try {
			...
			paramMap = getMockAlipayParams(list, bizPropertyMap);
			// 建立请求
			String sHtmlText = AlipaySubmit.buildRequest(paramMap, "post", "确认",
					TradeUtil.getMchPrivateKey(bizPropertyMap));
