 ...
 */
@Service
public class GetStoreByCoordinatesAction extends TransAction{
    ...
    protected ShopResponse doTransaction(RequestContext context) throws ShopException {
        ...
        if(!Strings.isNullOrEmpty(supportRecovery)&&supportRecovery.equals("1")){
            StoreDTO storeDTO = storeManager.getRecoveryStoreByCoordinates(sellerId, longitude, latitude, bizCode);
            return ResponseUtil.getSuccessResponse(storeDTO);
        }else if(!Strings.isNullOrEmpty(supportRepair)&&supportRepair.equals("1")){
            StoreDTO storeDTO = storeManager.getRepairStoreByCoordinates(sellerId, longitude, latitude, bizCode);
            return ResponseUtil.getSuccessResponse(storeDTO);
        }else {
    ...
    public String getName() {
        return ActionEnum.GET_STORE_BY_COORDINATES.getActionName();
    }
}
