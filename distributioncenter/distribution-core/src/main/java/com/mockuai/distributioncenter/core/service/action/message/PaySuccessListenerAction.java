...
public class PaySuccessListenerAction implements Action {
    ...
    public DistributionResponse execute(RequestContext context) throws DistributionException {
        ...
        Message msg = paySuccessMsg.getMsg();
        // 检查消息是否别消费过，消息去重
        MessageRecordDTO messageRecordDTO = messageRecordManager.getRecordByIdentifyAndBizType(
                String.valueOf(paySuccessMsg.getDistributionOrderDTO().getOrderId()), BizType.TRADE_PAY_SUCCESS.getValue()
        );
        ...
        if (distRecordDTOs != null && !distRecordDTOs.isEmpty()) {
        	...
        	if(distributionOrderDTO.getPaymentId() != 12){
	            ...
	            messageRecordDTO.setBizType(BizType.TRADE_PAY_SUCCESS.getValue());
	            messageRecordDTO.setDeleteMark(distributionOrderDTO.getPaymentId());
            ...
            for (DistRecordDTO dto : distRecordDTOs) {
                ...
                grantedWealthDTO.setReceiverId(dto.getSellerId());
                ...
                if (dto.getSource().equals(DistributeSource.NOSHARE_DIST.getSource())) {
                	...
                	grantedWealthDTO.setSourceType(SourceType.NOSHARE_DIST.getValue());
                	
                }else if (dto.getSource().equals(DistributeSource.SHARE_DIST.getSource())){
                	
                	grantedWealthDTO.setSourceType(SourceType.SHARE_DIST.getValue());
                	
                }else if (dto.getSource().equals(DistributeSource.NOPURCHASE_DIST.getSource())){
                	
                	grantedWealthDTO.setSourceType(SourceType.NOPURCHASE_DIST.getValue());
                	
                }else if (dto.getSource().equals(DistributeSource.PURCHASE_DIST.getSource())){
                	
                	grantedWealthDTO.setSourceType(SourceType.PURCHASE_DIST.getValue());
...
//
//                giftsManager.grantActionGift(sellerDTO.getUserId(), sellerDTO.getUserName(), 3, 3, appKey);
//                
//                // 异步处理直接下级的数量和团队的数量
