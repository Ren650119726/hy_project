...
@Service
public class ReturnStoreSkuStockByOrderSkuAction extends TransAction {
    private static final Logger log = LoggerFactory
            .getLogger(ReturnStoreSkuStockByOrderSkuAction.class);
    ...
    protected SupplierResponse doTransaction(RequestContext context)
        ...
        try {
            supplierOrderStockManager.returnStoreSkuStockBySku(orderStockDTO, appKey);
            log.info(" ReturnStoreSkuStock  success, Exit Action [{}]", getName());
            return new SupplierResponse(true);
        } catch (SupplierException e) {

            if (e.getResponseCode().equals(ResponseCode.B_RETURNSTORESKUSTOCKBYSKU_ERROR)) {
                return new SupplierResponse(true);
            }
    ...
    public String getName() {
        ...
        return ActionEnum.RETURN_STORESKUSTOCKBYSKU.getActionName();
    }
}
