...
import com.mockuai.suppliercenter.common.constant.ParamEnum;
...
import com.mockuai.suppliercenter.core.filter.FilterChain;
import com.mockuai.suppliercenter.core.filter.FilterManager;
import com.mockuai.suppliercenter.core.service.action.Action;
import com.mockuai.suppliercenter.core.service.action.ActionHolder;
...
public class RequestDispatcher {
    ...
    public SupplierResponse dispatch(SupplierRequest req) {
        if (req == null) {
    ...
    private String getAppCode(RequestContext context, SupplierRequest req) {
        // TODO impl
        String appCode = null;
     ...
     * </pre>
     */
    class ActionCall implements Callable<SupplierResponse> {

        private final SupplierRequest req;

        public ActionCall(SupplierRequest req) {
            super();
            this.req = req;
        }

        @Override
        public SupplierResponse call() {
            ...
            if (null != action) {
                ...
                SupplierResponse re = new SupplierResponse(true);
                // 获取appCode
                String appCode = getAppCode(context, req);
                if (!allowAccess(appCode, action)) {
                    re = new SupplierResponse(ResponseCode.B_REQUEST_FORBBIDEN);
                    return re;
                }
                ...
                try {
                    ...
                    // 1. 执行filter.before流程
                    SupplierResponse beforeFilterResult = filterChain.before(context);
                    ...
                    // 3. 执行filter.after流程
                    SupplierResponse afterFilterResult = filterChain.after(context);
                    ...
                    res = new SupplierResponse(e.getResponseCode(), e.getMessage());
                    return res;
                } catch (Exception e) {
                    log.error("do action:" + req.getCommand()
                            + " occur Exception:", e);
                    res = new SupplierResponse(ResponseCode.SYS_E_SERVICE_EXCEPTION);
                    return res;
                } finally {
                    accessLogger.info("command:{}, result:{}, filterBeforeCost:{}, actionCost:{}, filterAfterCost:{}",
                            req.getCommand(),
                            res.isSuccess(),
