...
@Service
public class ThawItemSkuStockAction implements Action {
	private static final Logger log = LoggerFactory.getLogger(ThawItemSkuStockAction.class);
	@Resource
	...
	public ItemResponse execute(RequestContext context) throws ItemException {
		...
		if (request.getInteger("number") == null) {
			return ResponseUtil.getErrorResponse(ResponseCode.PARAM_E_MISSING, "thawNumber is missing");
		}
		...
		try {
			itemSkuManager.thawItemSkuStock(skuId, sellerId, increasedNumber, bizCode);
	...
	public String getName() {
		return ActionEnum.THAW_ITEM_SKU_STOCK.getActionName();
	}
}
