...
@Service
public class GetSimpleItemAction implements Action {
    private static final Logger log = LoggerFactory.getLogger(GetSimpleItemAction.class);
    ...
    private final Long sellerId = 1841254L;
...
//				List<ItemImageDTO> itemExtraImageList = getItemExtraImageList(itemImageDTOList);
               ...
               //读取相关组合商品
               List<CompositeItemDTO> compositeItemList =   compositeItemManager.getCompositeItemByItemId(itemId);
               if(CollectionUtils.isNotEmpty(compositeItemList)){
                   itemDTO.setCompositeItemList(compositeItemList);
               }
...
//            log.error("xxxx item_uid {}_{}",sellerId,itemId);
    ...
    public String getName() {
        return ActionEnum.GET_SIMPLE_ITEM.getActionName();
    }
}
