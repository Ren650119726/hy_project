...
@Service
public class ThawOrderSkuStockPartiallyAction extends TransAction {
    private static final Logger log = LoggerFactory.getLogger(ThawOrderSkuStockPartiallyAction.class);
    @Resource
    ...
    protected ItemResponse doTransaction(RequestContext context) throws ItemException {
        ...
        for (OrderStockDTO.OrderSku orderSku : orderStockDTO.getOrderSkuList()) {
            ...
            if (!recordSkuMap.keySet().contains(skuId)) {
                ...
                return ResponseUtil.getErrorResponse(ResponseCode.BASE_PARAM_E_RECORD_NOT_EXIST, "sku" + skuId + "对应的库存冻结记录不存在", failure);
            }
        ...
        stockFrozenRecordManager.updateStockFrozenRecordStatus(orderSn, skuIdList, sellerId, StockFrozenRecordStatusEnum.THAWY.getStatus(), bizCode);
    ...
    public String getName() {
        return ActionEnum.THAW_ORDER_SKU_STOCK_PARTIALLY.getActionName();
    }
}
