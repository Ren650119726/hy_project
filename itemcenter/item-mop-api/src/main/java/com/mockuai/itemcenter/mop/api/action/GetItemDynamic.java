 ...
 */
public class GetItemDynamic extends BaseAction {
	private static final Logger log = LoggerFactory
			.getLogger(GetItemDynamic.class);
	...
	public MopResponse execute(Request request) {
		...
		/*
		 * String distributorIdStr = (String)
		 * request.getParam("distributor_id");
		 * 
		 * if (StringUtils.isBlank(distributorIdStr) == false) { distributorId =
		 * Long.valueOf(distributorIdStr); }
		 */

		ItemUidDTO itemUidDTO = MopApiUtil.parseItemUid(itemUidStr);
		if (itemUidDTO == null) {
			return new MopResponse(ResponseCode.PARAM_E_INVALID.getCode(),
					"item_uid is invalid");
		}

		/*
		 * if (distributorId == null) { return new
		 * MopResponse(MopRespCode.P_E_PARAM_ISNULL, "distributor_id不能为空"); }
		 */

		com.mockuai.itemcenter.common.api.Request itemReq = new BaseRequest();
		itemReq.setCommand(ActionEnum.GET_ITEM_DYNAMIC.getActionName());
		itemReq.setParam("id", itemUidDTO.getItemId());
		itemReq.setParam("supplierId", itemUidDTO.getSellerId());
		itemReq.setParam("userId", userId);
		if (itemTypeStr != null && !"".equals(itemTypeStr)) {
			itemReq.setParam("itemType", Integer.parseInt(itemTypeStr));
		}
		...
		try {
			...
			// log.info(" @@@ itemResp :"+JsonUtil.toJson(itemResp));
			if (itemResp.getCode() == ResponseCode.SUCCESS.getCode()) {
				Map<String, Object> data = new HashMap<String, Object>();
				MopItemDTO mopItemDTO = MopApiUtil.genMopItem(itemResp
						.getModule());
				...
				try {
					...
					itemReqx1.setCommand(ActionEnum.QUERY_ITEM_DISCOUNT_INFO
							.getActionName());
					Response<MopCombineDiscountDTO> response = getItemService()
							.execute(itemReqx1);

					if (response.getModule() != null) {
						/*BeanUtils.copyProperties(response.getModule(),
								mopItemDTO);*/
						com.mockuai.itemcenter.mop.api.util.BeanUtils.copyPropertiesIgnoreNull(response.getModule(), mopItemDTO);
						// 第三个时间点,优惠信息
						point2 = DateTime.now();
						// 兼容老版本
						/*
						 * List<MopDiscountInfo> discountInfoList =
						 * Lists.newArrayList(); MopCombineDiscountDTO
						 * mopCombineDiscountDTO = response.getModule(); if
						 * (!CollectionUtils
						 * .isEmpty(mopCombineDiscountDTO.getCompositeActivityList
						 * ())) { discountInfoList.addAll(mopCombineDiscountDTO.
						 * getCompositeActivityList()); } if
						 * (!CollectionUtils.isEmpty
						 * (mopCombineDiscountDTO.getDiscountInfoList())) {
						 * discountInfoList
						 * .addAll(mopCombineDiscountDTO.getDiscountInfoList());
						 * } if (!CollectionUtils.isEmpty(mopCombineDiscountDTO.
						 * getTimeLimitList())) {
						 * discountInfoList.addAll(mopCombineDiscountDTO
						 * .getDiscountInfoList()); }
				...
				if (userId != null) {
					...
					itemReq.setCommand(ActionEnum.QUERY_ITEM_COLLECTION
							.getActionName());
					itemReq.setParam("itemCollectionQTO", itemCollectionQTO);
					itemReq.setParam("appKey", appKey);
					Response<List<ItemDTO>> queryItemCollectResp = this
							.getItemService().execute(itemReq);
					List<PropertyDTO> bizPropertyDTOList = mopItemDTO
							.getBizPropertyList();
					...
					if (queryItemCollectResp.getCode() == ResponseCode.SUCCESS
							.getCode()
				...
				log.info(
						"item get time statistics start {} point1 {} point2 {} point3 {} interval1 {} interval2 {} interval 3 {}",
						new Object[] { start.toString(format),
								point1.toString(format),
								point2.toString(format),
								point3.toString(format),
								point1.getMillis() - start.getMillis(),
								point2.getMillis() - point1.getMillis(),
								point3.getMillis() - point2.getMillis() });
				return new MopResponse(data);
			} else {
				return new MopResponse(itemResp.getCode(),
						itemResp.getMessage());
			}
