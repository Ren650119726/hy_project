 ...
 */
@Service
public class AddHeadSingleSubAction implements Action {
	@Resource
	...
	public HeadSingleResponse execute(RequestContext context) throws HeadSingleException {
    	...
    	if(null == headSubDTO){
    		return new HeadSingleResponse(ResponseCode.ADD_HEADSINGLE_NULL, "新增首单立减信息为空");
    	}
    	
    	HeadSingleSubDTO headSingleSubDTO = headSingleSubManager.addHeadSingleSub(headSubDTO);

        return HeadSingleUtils.getSuccessResponse(headSingleSubDTO);
    }

    public String getName() {
        return ActionEnum.ADD_HEADSINGLE_SUB.getActionName();
    }
}